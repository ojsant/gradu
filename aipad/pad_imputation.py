...
def form_test_matrices(model: _BaseImputer, X_test: np.ndarray, y_test: np.ndarray, transpose=False) -> list:   # TODO think of a better name
    true = X_test
    test = y_test
...
def calculate_scores(model: _BaseImputer, res: list) -> list:
    ...
    for i, _ in enumerate(pred):
        if np.any(np.isfinite(target[i])) and np.any(np.isfinite(pred[i])):
            scores.append(r2_score(target[i][np.isfinite(target[i])], pred[i][np.isfinite(pred[i])]))
            mse_scores.append(-mean_squared_error(target[i][np.isfinite(target[i])], pred[i][np.isfinite(pred[i])]))
        else:
...
def plot_results(model: _BaseImputer, res: list, intensities: np.ndarray, sc: WindConstants, cov_sc: SoloConstants, transpose=True, save_plot=True, save_path=None) -> tuple:
    scores, mse_scores = calculate_scores(model, res)

    true, reduced, pred_full, pred, target = res
    true_miss_percent = np.sum(np.where(np.isnan(true), 1, 0)) / (true.shape[0] * true.shape[1]) * 100
    reduced_miss_percent = np.sum(np.where(np.isnan(reduced), 1, 0)) / (reduced.shape[0] * reduced.shape[1]) * 100
    ...
    if isinstance(model, KNNImputer):
        ...
        axs[3].set_title(f"Mean imputed values")
